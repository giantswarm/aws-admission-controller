...
- In a `Cluster` resource, the  release version label can only be changed to a version that is greater than the current one but does   
  not skip major versions by admin users and users in restricted groups. 
