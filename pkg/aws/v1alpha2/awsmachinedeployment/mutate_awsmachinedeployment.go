...
func (m *Mutator) MutateAvailabilityZones(awsMachineDeployment infrastructurev1alpha2.AWSMachineDeployment) ([]mutator.PatchOperation, error) {
	...
	awsControlPlane, err := awsv1alpha2.FetchAWSControlPlane(&awsv1alpha2.Handler{K8sClient: m.k8sClient, Logger: m.logger}, &awsMachineDeployment)
	if err != nil {
		return nil, microerror.Mask(err)
	}
	...
	// We default the AZs
	defaultedAZs := awsv1alpha2.GetNavailabilityZones(&awsv1alpha2.Handler{K8sClient: m.k8sClient, Logger: m.logger}, awsv1alpha2.DefaultNodePoolAZs, awsControlPlane.Spec.AvailabilityZones)
...
func (m *Mutator) MutateOperatorVersion(awsMachineDeployment infrastructurev1alpha2.AWSMachineDeployment) ([]mutator.PatchOperation, error) {
	...
	awsCluster, err := awsv1alpha2.FetchAWSCluster(&awsv1alpha2.Handler{K8sClient: m.k8sClient, Logger: m.logger}, &awsMachineDeployment)
	if err != nil {
		return nil, microerror.Mask(err)
	}

	// mutate the operator label
	patch, err = awsv1alpha2.MutateLabelFromAWSCluster(&awsv1alpha2.Handler{K8sClient: m.k8sClient, Logger: m.logger}, &awsMachineDeployment, *awsCluster, label.AWSOperatorVersion)
	if err != nil {
		return nil, microerror.Mask(err)
	}
	result = append(result, patch...)
...
func (m *Mutator) MutateReleaseVersion(awsMachineDeployment infrastructurev1alpha2.AWSMachineDeployment) ([]mutator.PatchOperation, error) {
	...
	cluster, err := awsv1alpha2.FetchCluster(&awsv1alpha2.Handler{K8sClient: m.k8sClient, Logger: m.logger}, &awsMachineDeployment)
	if err != nil {
		return nil, microerror.Mask(err)
	}

	// mutate the release label
	patch, err = awsv1alpha2.MutateLabelFromCluster(&awsv1alpha2.Handler{K8sClient: m.k8sClient, Logger: m.logger}, &awsMachineDeployment, *cluster, label.Release)
	if err != nil {
		return nil, microerror.Mask(err)
	}
	result = append(result, patch...)
