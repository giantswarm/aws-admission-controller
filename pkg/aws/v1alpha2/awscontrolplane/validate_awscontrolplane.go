...
func (v *Validator) Validate(request *admissionv1.AdmissionRequest) (bool, error) {
	...
	err = awsv1alpha2.ValidateOrganizationLabelContainsExistingOrganization(context.Background(), v.k8sClient.CtrlClient(), &awsControlPlane)
	if err != nil {
		return false, microerror.Mask(err)
	}
	...
	g8sControlPlane, err = awsv1alpha2.FetchG8sControlPlane(&awsv1alpha2.Handler{K8sClient: v.k8sClient, Logger: v.logger}, &awsControlPlane)
...
func (v *Validator) AZCount(awsControlPlane infrastructurev1alpha2.AWSControlPlane) error {
	if !awsv1alpha2.IsValidMasterReplicas(len(awsControlPlane.Spec.AvailabilityZones)) {
		v.logger.Log("level", "debug", "message", fmt.Sprintf("AWSControlPlane %s has an invalid count of %v availability zones. Valid AZ counts are: %v",
			key.ControlPlane(&awsControlPlane),
			len(awsControlPlane.Spec.AvailabilityZones),
			awsv1alpha2.ValidMasterReplicas()),
		)
