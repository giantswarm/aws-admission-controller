...
func TestMutateOperatorVersion(t *testing.T) {
	...
	for i, tc := range testCases {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			releaseVersion, err := awsv1alpha2.ReleaseVersion(cluster.GetObjectMeta(), patch)
			if err != nil {
				t.Fatal(err)
			}
			...
			// parse patches
			for _, p := range patch {
				if p.Path == fmt.Sprintf("/metadata/labels/%s", awsv1alpha2.EscapeJSONPatchString(label.ClusterOperatorVersion)) {
