...
func TestMutateCopyCiliumCidrAnnotation(t *testing.T) {
	testCases := []struct {
		ctx  context.Context
		name string
		...
		{
			// Copies annotation from cluster when not set
			name: "case 0",
			ctx:  context.Background(),

			clusterAnnotation:  "blah",
			currentAnnotation:  "",
			expectedAnnotation: "blah",
		},
		{
			// Does nothing when annotation is already set
			name: "case 1",
			ctx:  context.Background(),

			clusterAnnotation:  "blah",
			currentAnnotation:  "exist",
			expectedAnnotation: "",
		},
		{
			// Does nothing when cluster has no annotation
			name: "case 2",
			ctx:  context.Background(),
	...
	for i, tc := range testCases {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			annotations := map[string]string{}
			if tc.clusterAnnotation != "" {
				annotations[annotation.CiliumPodCidr] = tc.clusterAnnotation
			}
			cluster.Annotations = annotations
			_ = fakeK8sClient.CtrlClient().Create(tc.ctx, cluster)
			...
			if tc.currentAnnotation != "" {
				annotations[annotation.CiliumPodCidr] = tc.currentAnnotation
			}
			awscluster.Annotations = annotations
			patch, err = mutate.MutateCopyCiliumCidrAnnotation(*awscluster)
			if err != nil {
				t.Fatal(err)
			}
			// parse patches
			...
			// check if the pod CIDR is as expected
			if tc.expectedAnnotation != cidr {
				t.Fatalf("expected %#q to be equal to %#q", tc.expectedAnnotation, cidr)
			}
		})
	}
}
